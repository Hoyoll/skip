...
use skip::{
    Center, Circle, Div, End, Font, Horizontal, Hover, Inc, Leak, Mouse, Mouses, Plain, Proc, Set,
    State, Text, Vec2, Vertical, X, XY, Y,
};
...
impl TextInput {
    pub fn accept(&mut self, button: &Button) {
        match button {
            ...
            Button::Action("=") => match meval::eval_str(&self.text) {
                Ok(val) => {
...
const BORDER: skip::cn::Border<Color, (f32, f32), (f32, f32)> =
    skip::cn::Border(Color::Fg, (1.0, 1.0), (0.0, 0.0));
...
impl AppController<Message> for Calc {
        ...
        ui.add(|background: Div<_>| {
            background
                ...
                .on::<Hover>(|(pos, background)| {
                    background.child::<Circle<_>, Leak>(|circle| {
                        circle
                            .position::<Set>(&pos)
                            .radius(height / 2.0)
                            .render(Color::Light)
                            ...
                            layout
                                ...
                                .iter((BUTTONS.iter(), 4), |button: Div<_>, btn| {
                                    ...
                                    button
                                        ...
                                        .proc(BORDER)
                                        .render::<Plain<_>>(Color::UiBg)
                                        .on::<Mouses>(|on| match on {
                                            (Mouse::Left, State::Pressed) => {
                                                self.context.text_input.accept(btn)
                                            }
                                            _ => (),
                                        })
                                        .on::<Hover>(|(_, div)| {
    ...
    fn bootstrap(&mut self, mut context: skip_skia::Context, proxy: EventLoopProxy<Message>) {
        let attr =
            winit::window::WindowAttributes::default().with_title(self.context.title.clone());
